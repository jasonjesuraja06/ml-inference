...
Three knobs (set via environment, read at process start):
  MODEL_VARIANT   quantized | fp32   selects which ONNX model directory to load
...
class Engine:
    def __init__(self) -> None:
        ...
        self.cache = LRUEmbeddingCache() if self.cache_enabled else None

        # Dynamic batching queue
        self._batch_queue: asyncio.Queue[tuple[str, asyncio.Future]] = asyncio.Queue()
        self._batch_window_s = float(os.environ.get("BATCH_WINDOW_MS", "8")) / 1000.0
        self._batch_max = int(os.environ.get("BATCH_MAX", "16"))
    ...
    async def _batch_loop(self) -> None:
        while True:
            ...
            while len(batch_codes) < self._batch_max:
