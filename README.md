...
|---|---|---|---|---|---|---|
| FP32, no cache, no batching | 44.1 | 790 ms | 1100 ms | 1400 ms | n/a | 0 / 2386 |
| INT8, no cache, no batching | 43.8 | 810 ms | 1100 ms | 1300 ms | n/a | 0 / 2351 |
| INT8 + cache | 109.7 | 82 ms | 450 ms | 580 ms | 66.4% | 0 / 5919 |
| INT8 + micro-batching | 41.7 | 880 ms | 1300 ms | 1800 ms | n/a | 0 / 2225 |
| INT8 + cache + micro-batching | 107.1 | 67 ms | 590 ms | 780 ms | 65.9% | 0 / 5820 |

Against the INT8 no-cache no-batching row: the cache alone is **2.50x**, batching alone is 0.95x, and both together are 2.45x, 0.98x of the cache by itself. Quantization contributes 0.99x in the serving path, matching the 1.03x it gets at batch size 1 on this architecture: dynamic INT8 does not pay off on arm64 in either measurement. Splitting the configurations five ways rather than comparing an optimized pair against a baseline is what makes those attributions possible; a blended number would have credited the cache's work to quantization. The batcher neither helps nor hurts much here, and the reason it cannot help is structural: a batcher pays off when the compute unit idles at batch size 1, but ONNX Runtime already spreads a 256-token pass across every core, so a batch does proportionally more work rather than filling idle capacity. The forward pass also runs on the event loop thread, so while a batch executes nothing else is served, including cache hits that would return in microseconds. At 4.16 mean batch size against a ceiling of 16 the batcher was only partly engaged, so this run does not test it at saturation. The measured best configuration on this host is `MODEL_VARIANT=quantized NO_BATCHING=1`. The batcher stays because the argument for it is hardware-dependent in the same way quantization is, and `/predict/batch` uses the same path for callers that genuinely have 64 functions at once. The 66.4% hit rate at a 30% repeat rate is a property of synthetic traffic, not a claim about real scanners.
